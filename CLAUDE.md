...
Si el usuario dice "implementá", "hacé el cambio", "codificá esto" o similar **sin haber pasado por `/new` y `/ff` primero**, respondé:

> "Para mantener la trazabilidad de SpecBoot, primero necesitamos crear el change de OpenSpec. ¿Continuamos con `/new <TICKET-ID>`?"
...
|------|---------|--------------------------|
| 1 | `/enrich-us <TICKET-ID>` | "✅ US enriquecida. ¿Continuamos con `/new <TICKET-ID>`?" |
| 2 | `/new <TICKET-ID>` | "✅ Propuesta creada. ¿Continuamos con `/ff <TICKET-ID>` para generar el diseño y las tareas?" |
| 3 | `/ff <TICKET-ID>` | "✅ Design y tareas generadas. ¿Continuamos con `/apply <TICKET-ID>` para implementar?" |
| 4 | `/apply <TICKET-ID>` | "✅ Implementación completada. Se ejecutará `/security-review` automáticamente. Luego continuamos con `/verify <TICKET-ID>`." |
| 5 | `/verify <TICKET-ID>` + `/save-verify-report <TICKET-ID>` | "✅ Verificación guardada. ¿Continuamos con `/adversarial-review <TICKET-ID>`?" |
| 6 | `/adversarial-review <TICKET-ID>` | "✅ Review adversarial completado. ¿Continuamos con `/save-qa-report <TICKET-ID>`?" |
| 7 | `/save-qa-report <TICKET-ID>` | "✅ Reporte QA guardado. ¿Continuamos con `/archive <TICKET-ID>`?" |
| 8 | `/archive <TICKET-ID>` | "✅ Change archivado. ¿Continuamos con `/commit`?" |
| 9 | `/commit` | "✅ PR creado. El flujo del ticket está completo." |
...
- **Si el usuario ya ejecutó un paso** en esta sesión (ej: ya corrió `/new`), reconocelo y avanzá al siguiente.
