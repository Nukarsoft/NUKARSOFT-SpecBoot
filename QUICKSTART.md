...
|---|---------|------------|---------------------------|
| 1 | `git clone https://github.com/<tu-org>/<tu-specboot>.git`<br>**Windows (cmd/PowerShell):** `robocopy <carpeta-clonada> tu-proyecto /E /XD .git`<br>**Mac/Linux:** `cp -rn <carpeta-clonada>/. tu-proyecto/` | Copia `docs/`, `ai-specs/`, `AGENTS.md`/`CLAUDE.md`/`codex.md`/`GEMINI.md` y las carpetas `.claude/`·`.cursor/` dentro de tu proyecto. ⚠️ **No uses `cp -rn <carpeta>/*`** en ningún OS: el glob `*` **no copia dotfolders** como `.claude/` (que contiene los slash commands `/setup-docs`, `/commit`, etc.). | Archivos nuevos sin commitear en el working tree de tu proyecto (`git status`), incluyendo `.claude/commands/` |
| 2 | `npm install -g @fission-ai/openspec@latest`<br>`cd tu-proyecto && openspec init` | Crea la carpeta `openspec/` (`config.yaml`, `changes/`, `specs/`) y habilita los comandos `/new`, `/ff`, `/apply`, `/verify`, `/archive` | `openspec/config.yaml` en tu proyecto |
