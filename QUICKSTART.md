...
| 2 | `git clone https://github.com/<tu-org>/<tu-specboot>.git C:\temp\specboot-temp`<br><br>**Windows (cmd):**<br>`robocopy C:\temp\specboot-temp tu-proyecto /E /XD .git`<br>`robocopy C:\temp\specboot-temp\.claude tu-proyecto\.claude /E`<br>`robocopy C:\temp\specboot-temp\.cursor tu-proyecto\.cursor /E`<br><br>**Mac/Linux:**<br>`cp -rn /tmp/specboot-temp/. tu-proyecto/`<br><br>⚠️ En Windows, robocopy omite dotfolders (`.claude/`, `.cursor/`) en el primer comando — los dos comandos adicionales los copian explícitamente.<br>⚠️ Debe ejecutarse **DESPUÉS de `openspec init`**: si se hace antes, `openspec init` pisa los comandos de specboot.<br><br>Verificación (debe mostrar `setup-docs.md`, `commit.md`, etc.):<br>`dir tu-proyecto\.claude\commands\` | Archivos nuevos sin commitear en tu proyecto (`git status`), incluyendo `.claude/commands/setup-docs.md`, `commit.md`, etc. junto a la carpeta `opsx/` de openspec. |
