...
|---|---------|------------|---------------------------|
| 1 | `git clone https://github.com/<tu-org>/<tu-specboot>.git`<br>`cp -rn <carpeta-clonada>/* tu-proyecto/` | Copia `docs/`, `ai-specs/`, `AGENTS.md`/`CLAUDE.md`/`codex.md`/`GEMINI.md` y los symlinks `.claude/`·`.cursor/` dentro de tu proyecto | Archivos nuevos sin commitear en el working tree de tu proyecto (`git status`) |
